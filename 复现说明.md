...
功能：加载（Safe）LoRA adapter，在 `datasets/samsum_test.jsonl` 上推理并统计 ROUGE。默认 `--max_samples=-1`，表示全量测试集评估。
...
此脚本会自动执行：依赖安装 → LoRA 微调 → SafeLoRA 投影 → SAMSum ROUGE 评估 → SST2 基础模型评估。

脚本内已默认配置：

- Base 模型：`/data_nvme1n1/xieqiuhao/tjy/downloaded_models/Llama-2-7b-hf`
- Chat 模型：`/data_nvme1n1/xieqiuhao/tjy/downloaded_models/Llama-2-7b-chat-hf`

默认即“完整复现”配置：

- 训练使用完整 `datasets/samsum_1000_bad.jsonl`（1100 条）。
- 评估使用完整 `datasets/samsum_test.jsonl`（不截断，`MAX_EVAL_SAMPLES=-1`）。
- 额外评估未微调前基础（chat）模型在 SST2 `validation` 全量上的准确率。

新增输出文件：

- `outputs/reproduce/sst2_base_metrics_*.json`：基础模型 SST2 评估详情与准确率。
