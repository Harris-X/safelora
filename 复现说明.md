...
文件：`scripts/run_reproduce.ps1`

此脚本会自动执行：依赖安装 → LoRA 微调 → SafeLoRA 投影 → ROUGE 评估。

```powershell
powershell -ExecutionPolicy Bypass -File scripts/run_reproduce.ps1 `
  -BaseModelPath D:/models/Llama-2-7b-base `
  -ChatModelPath D:/models/Llama-2-7b-chat `
  -UseFp16
```

可选参数：

- `-TrainFile`（默认 `datasets/samsum_1000_bad.jsonl`）
- `-TestFile`（默认 `datasets/samsum_test.jsonl`）
- `-OutRoot`（默认 `outputs/reproduce`）
- `-MaxEvalSamples`（默认 `200`）
